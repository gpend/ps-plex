...
function processFile {
    ...
    ($moveFile) ? (& { $command = 'Move-Item'; $resultText = 'moved' }) :
     (& { $command = 'copy-Item'; $resultText = 'coppied' })
    ($isTest) ? ($testCommand = "-whatIf") : ($testCommand = "")
    
    # rename and move stuff
    $episodeNumber = "{0:d2}" -f $episodeNumber
    $sequence = "$showName - S$($seasonNumber)E$episodeNumber"
    &"$($command) -Path $($file.FullName) -destination $($seasonPath)\$($sequence.mkv) $($testCommand)"
    $episodeNumber = [int]$episodeNumber
    $episodeNumber ++
    "File $($file.FullName) renamed and $($resultText) to $($seasonPath)$($sequence).mkv" | Out-File -FilePath $logFile -Append
}
